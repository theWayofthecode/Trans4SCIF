...
struct ITestCaseRegistry {
  ...
  virtual void getFilteredTests
      (TestSpec const &testSpec, IConfig const &config, std::vector<TestCase> &matchingTestCases, bool negated = false)
      const = 0;
...
#pragma clang diagnostic pop
  ...
  struct Filter {
    ...
    bool matches(TestCaseInfo const &testCase) const {
      ...
      for (std::vector<Ptr<Pattern> >::const_iterator it = m_patterns.begin(), itEnd = m_patterns.end(); it != itEnd;
           ++it)
...
class BoundArgFunction {
 public:
  ...
  BoundArgFunction(BoundArgFunction const &other) : functionObj(other.functionObj ? other.functionObj->clone()
                                                                                  : NULL) { }
...
struct AssertionStats {
  AssertionStats(AssertionResult const &_assertionResult,
    ...
    if (assertionResult.hasMessage()) {
      ...
      MessageBuilder
          builder(assertionResult.getTestMacroName(), assertionResult.getSourceInfo(), assertionResult.getResultType());
...
struct IReporter: IShared {
  ...
  virtual void EndTestCase
      (TestCaseInfo const &testInfo, Totals const &totals, std::string const &stdOut, std::string const &stdErr) = 0;
